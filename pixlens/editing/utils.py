...
def generate_description_based_prompt(edit: Edit) -> str:
    ...
    prompt_templates: dict[EditType, str] = {
        ...
        EditType.COLOR: (
            ...
            f"[SEP]A photo of a {to_attribute} {category}"
        ),
        EditType.SIZE: (
            f"A photo of a {category}"
            f"[SEP]A photo of a {to_attribute} {category}"
        ...
        EditType.OBJECT_REMOVAL: (
            f"A photo of a {category}"
            f"[SEP]A photo without a {category}"  # FIXME: this one is HARD!
        ),
        EditType.OBJECT_REPLACEMENT: (
            f"A photo of a {from_attribute}[SEP]A photo of a {to_attribute}"
        ),
        EditType.POSITION_REPLACEMENT: (
            f"A photo of a {category} on the {from_attribute}"
            f"[SEP]A photo of a {category} on the {to_attribute}"
        ),
        EditType.OBJECT_DUPLICATION: (
            f"A photo of a {category}[SEP]A photo of two {category}s"
        ),
        EditType.TEXTURE: (
            f"A photo of a {category}"
            f"[SEP]A photo of a {category} with {to_attribute} texture"
        ),
        EditType.ACTION: (
            f"A photo of a {category} {change_action_dict[from_attribute]}"
            f"[SEP]A photo of a {category} {change_action_dict[to_attribute]}"
        ...
        EditType.VIEWPOINT: (
            ...
            "[SEP]A photo of a {category} from {to_attribute}"
            if from_attribute is not None and not pd.isna(from_attribute)
            else (
                f"A photo of a {category}"
                f"[SEP]A photo of a {category} from {to_attribute}"
            )
        ),
        # also it could be (according to EDITVAL):
        # EditType.VIEWPOINT: (
        #    f"A photo of a {from_attribute} view of a {category}"
        #    f"[SEP]A photo of a {to_attribute} view of a {category}"
        # )
        EditType.BACKGROUND: (
            f"A photo of a {category} in the {from_attribute}"
            f"[SEP]A photo of a {category} in the {to_attribute}"
            if from_attribute is not None and not pd.isna(from_attribute)
            else (
                f"A photo of a {category}"
                f"[SEP]A photo of a {category} in the {to_attribute}"
        ...
        EditType.STYLE: (
            f"A photo of a {category}"
            f"[SEP]An art painting of a {category} in {to_attribute} style"
        ),
        EditType.SHAPE: (
            f"A photo of a {category}"
            f"[SEP]A photo of a {category} in the shape of a {to_attribute}"
        ),
        EditType.ALTER_PARTS: (
            ...
            f"A photo of a {category}[SEP]{to_attribute} to the {category}"
        ),
    }
