# Taken from the HF community examples
#	https://github.com/huggingface/diffusers/pull/6329
...
class NullTextPipeline(StableDiffusionPipeline):
    ...
    def ddim_inversion_loop(self, latent, context):
        ...
        with torch.no_grad():
            for i in range(0, self.num_inference_steps):
                ...
                noise_pred = self.unet(
                    latent, t, encoder_hidden_states=cond_embeddings)["sample"]
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                noise_pred_uncond = self.unet(
                    latents, t, encoder_hidden_states=uncond_embeddings[i])["sample"]
                noise_pred = self.unet(
                    latents, t, encoder_hidden_states=prompt_embeds)["sample"]
                noise_pred = noise_pred_uncond + guidance_scale * \
        ...
        image = self.image_processor.postprocess(
            image, output_type=output_type, do_denormalize=[
                True] * image.shape[0]
        )
        # Offload all models
